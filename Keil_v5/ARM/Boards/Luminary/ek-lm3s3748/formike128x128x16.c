...
Formike128x128x16Init(void)
{
    ...
    //
    ROM_GPIOPinTypeGPIOOutput(LCD_A0_BASE, LCD_A0_PIN);
    ROM_GPIOPinTypeGPIOOutput(LCD_WR_BASE, LCD_WR_PIN);
    ROM_GPIOPinTypeGPIOOutput(LCD_RD_BASE, LCD_RD_PIN);
    ROM_GPIOPinTypeGPIOOutput(LCD_BL_BASE, LCD_BL_PIN);
    ROM_GPIOPinTypeGPIOOutput(LCD_DATA_BASE, 0xff);

    //
    // Set the LCD control pins to their default values.
    //
    ROM_GPIOPinWrite(LCD_A0_BASE, LCD_A0_PIN, LCD_A0_PIN);
    ROM_GPIOPinWrite(LCD_WR_BASE, LCD_WR_PIN | LCD_RD_PIN,
                     LCD_WR_PIN | LCD_RD_PIN);
    ROM_GPIOPinWrite(LCD_BL_BASE, LCD_BL_PIN, 0);
    ROM_GPIOPinWrite(LCD_DATA_BASE, 0xff, 0x00);
