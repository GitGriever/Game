...
void setLights (unsigned short light, unsigned short on) {
  ...
  if (light & red) {
    ...
    if (on == 0)
     GLCD_DisplayChar(4, 5, 1, 0x80+0);                      /* Circle Empty */
    else
     GLCD_DisplayChar(4, 5, 1, 0x80+1);                      /* Circle Full  */
  }
  if (light & yellow) {
    GLCD_SetTextColor(Yellow);                               /* Set the Text Color */
    if (on == 0)
     GLCD_DisplayChar(5, 5, 1, 0x80+0);                      /* Circle Emptyn */
    else
     GLCD_DisplayChar(5, 5, 1, 0x80+1);                      /* Circle Full  */
  }
  if (light & green) {
    GLCD_SetTextColor(Green);                                /* Set the Text Color */
    if (on == 0)
     GLCD_DisplayChar(6, 5, 1, 0x80+0);                      /* Circle Empty */
    else
     GLCD_DisplayChar(6, 5, 1, 0x80+1);                      /* Circle Full  */
  }
  if (light & stop) {
    GLCD_SetTextColor(Red);                                  /* Set the Text Color */
    if (on == 0)
     GLCD_DisplayChar(5, 7, 1, 0x80+12);                     /* Wait Empty */
    else
     GLCD_DisplayChar(5, 7, 1, 0x80+13);                     /* Wait Full  */
  }
  if (light & walk) {
    GLCD_SetTextColor(Green);                                /* Set the Text Color */
    if (on == 0)
     GLCD_DisplayChar(6, 7, 1, 0x80+14);                     /* Walk Empty */
    else
     GLCD_DisplayChar(6, 7, 1, 0x80+15);                     /* Walk Full  */
  }
...
__task void lcd (void) {
  ...
  for (;;) {
    ...
    GLCD_DisplayString(1, 0, 1, "    RTX Traffic     ");
