...
const unsigned long led_mask[] = { PIO_PA0, PIO_PD20, PIO_PD21 };
...
void LED_Init(void) {
  ...
  PMC->PMC_PCER0 = ((1UL << ID_PIOA) |    /* enable PIOA clock                */
                    (1UL << ID_PIOD)  );  /* enable PIOD clock                */

  PIOA->PIO_PER  =
  PIOA->PIO_OER  =
  PIOA->PIO_PUDR =
  PIOA->PIO_OWER = (PIO_PA0);             /* Setup PIO_PA0 for LED            */

  PIOD->PIO_PER  =
  PIOD->PIO_OER  =
  PIOD->PIO_PUDR =
  PIOD->PIO_OWER = (PIO_PD20 | PIO_PD21); /* Setup PIO_PD20..21 for LEDs      */
