...
#include <stm32f0xx.h>                  /* STM32F0xx.h definitions            */
#include "GLCD.h"
...
#define PIN_CS      (1UL <<  6)
#define PIN_CLK     (1UL <<  3)
#define PIN_DAT     (1UL << 15)
...
#define LCD_CS(x)   ((x)        ? (GPIOE->BSRR   = (PIN_CS   <<    0)) : (GPIOE->BSRR   = (PIN_CS    <<   16)) );
#define LCD_CLK(x)  ((x)        ? (GPIOB->BSRR   = (PIN_CLK  <<    0)) : (GPIOB->BSRR   = (PIN_CLK   <<   16)) );
#define LCD_DAT(x)  ((x)        ? (GPIOE->BSRR   = (PIN_DAT  <<    0)) : (GPIOE->BSRR   = (PIN_DAT   <<   16)) );

#define DAT_MODE(x)  (x == OUT) ? (GPIOB->BSRR   = ((1ul<<2) <<    0)) : (GPIOB->BSRR   =  ((1ul<<2) <<   16)); \
                     (x == OUT) ? (GPIOE->MODER |= ( 1UL     << 2*15)) : (GPIOE->MODER &= ~( 3UL     << 2*15));
#define BUS_VAL()   ((GPIOE->IDR & PIN_DAT) != 0)
...
static __inline unsigned char spi_tran (unsigned char byte) {
  ...
  *((volatile uint8_t *)&(SPI1->DR)) = byte;   /* 8bit access because of 8 databits  */
  while (!(SPI1->SR & RXNE));           /* Wait for send to finish            */
  return (*((volatile uint8_t *)&(SPI1->DR))); /* 8bit access because of 8 databits  */
}
...
static unsigned short rd_id_man (void) {
  ...
  GPIOE->MODER   &= ~(( 3UL << (2*14)) |
                      ( 3UL << (2*15))  );
  GPIOE->MODER   |=  (( 1UL << (2*14)) |
                      ( 1UL << (2*15))  );       /* Gen. purpose output mode  */

  GPIOB->MODER   &= ~(( 3UL << (2* 3))   );
  GPIOB->MODER   |=  (( 1UL << (2* 3))   );      /* Gen. purpose output mode  */

  /* The direction of this voltage translator is controlled by IO PB2
    (IO PE15 is working as MOSI when PB2 is high or as MISO when PB2 is LOW)  */
  GPIOB->MODER   &= ~(( 3UL << (2* 2))   );
  GPIOB->MODER   |=  (( 1UL << (2* 2))   );      /* Gen. purpose output mode  */
  ...
  GPIOB->ODR     |=  (1UL <<    2);              /* set PB.2 to High          */

/* SPI1_SCK, SPI1_MISO, SPI1_MOSI are SPI pins                                */
  GPIOB->MODER   &= ~(( 3UL << (2* 3))  );
  GPIOB->MODER   |=  (( 2UL << (2* 3))  );       /* Alternate Function mode   */
  GPIOE->MODER   &= ~(( 3UL << (2*14)) |
                      ( 3UL << (2*15))  );
  GPIOE->MODER   |=  (( 2UL << (2*14)) |
                      ( 2UL << (2*15))  );       /* Alternate Function mode   */
...
void GLCD_Init (void) {
  ...
  /* Enable clock for GPIOB, E and SPI1                                       */
  RCC->AHBENR  |= (( 1UL << 21) | (1UL << 18));
  RCC->APB2ENR |=  ( 1UL << 12);

  /* NCS is PE6, GPIO output set to high                                      */
  GPIOE->ODR     |=  ( 1UL << (   6));           /* set Pin to High           */
  GPIOE->MODER   &= ~( 3UL << (2* 6));
  GPIOE->MODER   |=  ( 1UL << (2* 6));           /* Gen. purpose output mode  */

  /* SPI1_SCK, SPI1_MISO, SPI1_MOSI are SPI pins                              */
  GPIOB->OSPEEDR |=  (( 3UL << (2* 3)));
  GPIOE->OSPEEDR |=  (( 3UL << (2*14)) |
                      ( 3UL << (2*15))  );
  ...
  GPIOB->AFR[0]  &= ~((15UL << (4*( 3  )))  );   /* Alternate Function 0      */
  GPIOE->AFR[1]  &= ~((15UL << (4*(14-8))) |
                      (15UL << (4*(15-8)))  );
  GPIOE->AFR[1]  |=  (( 1UL << (4*(14-8))) |
                      ( 1UL << (4*(15-8)))  );   /* Alternate Function 1      */


  /* Enable SPI in Master Mode, CPOL=1, CPHA=1                                */
  /* Max. 12 MBit used for Data Transfer @ 48MHz                              */
  RCC->APB2RSTR |=  (1UL << 12);                 /* start SPI1 reset          */
  RCC->APB2RSTR &= ~(1UL << 12);                 /* stop  SPI1 reset          */

  SPI1->CR1  = ((1UL <<  0) |                    /* CPHA = 1                  */
                (1UL <<  1) |                    /* CPOL = 1                  */
                (1UL <<  3) |                    /* BR = 1   = fPCLK / 4      */
                (1UL <<  9)  );                  /* SSM = 1                   */

  SPI1->CR2  = ((7UL <<  8) |                    /* DS = 7   = 8bit           */
                (1UL << 12)  );                  /* FRXTH = 1                 */

  SPI1->CR1 |= ((1UL <<  2) |                    /* MSTR = 1                  */
                (1UL <<  8)  );                  /* SSI = 1                   */

  SPI1->CR1 |=  (1UL <<  6);                     /* SPE = 1                   */

  driverCode = rd_id_man ();
  if ((driverCode == 0x00) || (driverCode == 0xFF)) {
