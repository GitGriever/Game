...
/*                                  E_SYS_UART0_RST / E_SYS_UART1_RST / E_SYS_PWM03_RST / E_SYS_PS2_RST    */
/*                                  E_SYS_USBD_RST                                                         */
/*                                                                                                         */
/* Returns:                                                                                                */
/*               None                                                                                      */
...
/*                                  E_SYS_TMR2_CLK  / E_SYS_TMR3_CLK  / E_SYS_I2C_CLK   / E_SYS_SPI0_CLK   */
/*                                  E_SYS_SPI1_CLK  / E_SYS_UART0_CLK / E_SYS_UART1_CLK / E_SYS_PWM01_CLK  */
/*                                  E_SYS_PWM23_CLK / E_SYS_USBD_CLK  / E_SYS_PS2_CLK   / E_SYS_ISP_CLK    */
/*                                                                                                         */
/*               i32Enable  - [in]  1 : Enable / 0 : Disable                                               */
/* Returns:                                                                                                */
/*               None                                                                                      */
...
/*                                                                                                         */
/* Parameters:                                                                                             */
/*           u8ClkSrcSel       - [in]   0: External Crystal clock (4 ~ 24 MHz)                             */
...
/*                                                                                                         */
/* Parameters:                                                                                             */
/*              eIpClkSrc    - [in]     E_SYS_WDT_CLKSRC   / E_SYS_TMR0_CLKSRC  / E_SYS_TMR1_CLKSRC        */
...
/*                                                                                                         */
/* Parameters:                                                                                             */
/*           eIpDiv  - [in] E_SYS_UART_DIV / E_SYS_USBD_DIV / E_SYS_HCLK_DIV                               */
/*           u8value - [in] 0 ~ 15                                                                         */
/*                                                                                                         */
/* Returns:                                                                                                */
/*           0                      : Success                                                              */
...
/*               The actual clock divide number is (u8value + 1)                                           */
/*---------------------------------------------------------------------------------------------------------*/
int32_t DrvSYS_SetClockDivider(E_SYS_IP_DIV eIpDiv , uint8_t u8value)
{
    ...
    if (eIpDiv == E_SYS_UART_DIV)
    {
        SYSCLK->CLKDIV.UART_N = u8value;
    }
    else if (eIpDiv == E_SYS_USBD_DIV)
    {
        SYSCLK->CLKDIV.USB_N = u8value;
    }
    else if(eIpDiv == E_SYS_HCLK_DIV)
    {
        SYSCLK->CLKDIV.HCLK_N = u8value;
        SystemCoreClockUpdate();
    }
    else
...
/*                            E_DRVSYS_EXTERNAL_12M: External Crystal clock (4 ~ 24 MHz)                   */
/*                            E_DRVSYS_INTERNAL_22M: Internal 22M clock                                    */
/*      u32PllClk     - [in]  The target PLL clock frequency (Hz). The value is 25~500 (MHz)               */
/*                                                                                                         */
/* Returns:                                                                                                */
...
uint32_t DrvSYS_GetPLLContent(E_SYS_PLL_CLKSRC ePllSrc, uint32_t u32PllClk)
{
    ...
    if ( (u32PllClk < FREQ_125MHZ) && (u32PllClk >= FREQ_25MHZ) )
    {
        u32NO = 3;
        u32PllClk = u32PllClk << 2;
    }
    else if ( (u32PllClk < FREQ_250MHZ) && (u32PllClk >= FREQ_125MHZ) )
    {
        u32NO = 1;
        u32PllClk = u32PllClk << 1;
    }   
    else if ( (u32PllClk <= FREQ_500MHZ) && (u32PllClk >= FREQ_250MHZ) )
    {
        u32NO = 0;
    }
    else
...
{   
    ...
    if (SYSCLK->CLKSEL0.HCLK_S == 0)        /* External Crystal clock (4 ~ 24 MHz) */
    {
        u32Freqout = __XTAL;    
    }
...
/*                          The range of u32Hclk is 25 ~ 60 MHz                                            */
/* Returns:                                                                                                */
/*      E_SUCCESS                    Success                                                               */
...
/*                                                                                                         */
/* Parameters:                                                                                             */
/*               us     - [in]      Delay time. The Max value is 279000                                    */
/*                                                                                                         */
/* Returns:                                                                                                */
/*      None                                                                                               */
/*                                                                                                         */
...
/*                                                                                                         */
/* Description:                                                                                            */
/*               This function is used to monitor if the chip clock source is stable or not                */
/*---------------------------------------------------------------------------------------------------------*/
...
/*               when software switches system clock source.                                               */
/*---------------------------------------------------------------------------------------------------------*/
