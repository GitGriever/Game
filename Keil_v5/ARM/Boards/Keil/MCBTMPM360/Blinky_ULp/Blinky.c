/*----------------------------------------------------------------------------
 * Name:    Blinky.c
 * Purpose: LED Flasher for MCBTMPM360
 * Note(s):
 *----------------------------------------------------------------------------
...
#define LED_ALL   0xF0
const unsigned long led_mask[] = { 1UL<< 4,  1UL<< 5, 1UL<< 6, 1UL<< 7  };
...
/* variable to trace in LogicAnalyzer (should not read too often)             */
...
void ADC_init (void) {
  ...
  TSB_AD->MOD1 = (0 << 0) |             /* Input Channel: AIN0                */
                 (1 << 7) ;             /* VREF=ON                            */

  TSB_AD->CLK  = (8 << 4) |             /* 8 clocks for conversion            */
                 (1 << 0);              /* A/D prescaler = fc/2               */
  TSB_AD->MOD0 = (2 << 3);              /* generate interrupt once every 8 conversions */
...
void LED_On (unsigned int num) {
  ...
  TSB_PM->DATA &= ~led_mask[num];       /* Turn LED on                        */
}
...
void LED_Off (unsigned int num) {
  ...
  TSB_PM->DATA |=  led_mask[num];       /* Turn LED off                       */
}
