...
void LED_UnInit(void) {
  ...
  PORTC->PCR[ 8] = 0;                   /* PTC9  is at reset state            */
  PORTB->PCR[11] = 0;                   /* PTC11  is at reset state           */
}
...
void LED_On (uint32_t num) {
  ...
  if (num < NUM_LEDS) {
    if (num < 3) PTC->PCOR = led_mask[num];
    else PTB->PCOR = led_mask[num];
  }
}
...
void LED_Off (uint32_t num) {
  ...
  if (num < NUM_LEDS) {
    if (num < 3) PTC->PSOR = led_mask[num];
    else PTB->PSOR = led_mask[num];
  }
}
