...
void Out_Init (void) {
  ...
  GLCD_DisplayString(1, 0, 1, "      CAN_Ex2       ");
...
/*----------------------------------------------------------------------------
 *  Task 2: Sets answer to reception of remote transmission request periodically
 *---------------------------------------------------------------------------*/
...
__task void task_set_CAN (void)  {
  ...
  for (;;)  {
    ...
    CAN_set(1, &msg_set, 0x0F00);     /* Set data to answer on Remote request*/
...
/*----------------------------------------------------------------------------
 *  Task 3: Sends remote transmission request and receives response
 *---------------------------------------------------------------------------*/

__task void task_rece_CAN (void)  {
  CAN_msg msg_rtr        = { 33, {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},   
                              1, 2, STANDARD_FORMAT, REMOTE_FRAME };
  CAN_msg msg_rece;

  for (;;)  {
    CAN_request (1, &msg_rtr, 0x00FF);/* Send request on controller 1        */
