...
unsigned char In_Get (void) {
  ...
  while (!ADC_DoneCnv());               /* Wait until conversion done        */
  return (ADC_GetCnv() >> 4);           /* AD value (12 bit) => return 8-bit */
}
...
void Out_Init (void) {
  ...
  GLCD_DisplayString(0, 0, 1, " STM3220G-EVAL Demo ");
  ...
  GLCD_DisplayString(25, 0, 0, " Note: ");
  GLCD_DisplayString(26, 0, 0, " Values are not displayed on the LCD as some ");
  GLCD_DisplayString(27, 0, 0, " LCD pins are shared with CAN pins !!!       ");
...
void Out_Val (void) {
  ...
  val = Rx_val >> 4;    
  ((val & (1 << 0)) ? (GPIOG->BSRRL = (1 << 6)) : (GPIOG->BSRRH = (1 << 6)));
  ((val & (1 << 1)) ? (GPIOG->BSRRL = (1 << 8)) : (GPIOG->BSRRH = (1 << 8)));
  ((val & (1 << 2)) ? (GPIOI->BSRRL = (1 << 9)) : (GPIOI->BSRRH = (1 << 9)));
  ((val & (1 << 3)) ? (GPIOC->BSRRL = (1 << 7)) : (GPIOC->BSRRH = (1 << 7)));
}
