 ...
 *------------------------------------------------------------------------------
 *      Name:    CAN_MK60.c
 *      Purpose: CAN Driver, Hardware specific module for Freescale MK60
 *      Rev.:    V5.18a
 *------------------------------------------------------------------------------
...
static void CAN_hw_rd (U32 ctrl, U32 ch, CAN_msg *msg)  {
  ...
  while (cs & (1 << 24)) {              /* If BUSY, wait for BUSY to end      */
    cs = CANx->MB[ch].CS;
  }
...
//remote_type = (object_para &  FRAME_TYPE) == REMOTE_TYPE;
  ...
  CANx->RXIMR[ch] = 0xFFFFFFFFU;        /* Unmask all bits for check          */
