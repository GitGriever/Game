 ...
 *
 * SVN $Revision: 4754 $
 * SVN $Date: 2012-10-16 15:35:46 +0100 (Tue, 16 Oct 2012) $
 */
...
#define DRBG_RESET_REQUEST_CMD              45
...
#define DRBG_RESET_SERV_RESP_LENGTH         2
...
/*
 * Length of the commands received from the system controller.
 */
#define FLASHFREEZE_SHUTDOWN_CMD_LENGTH     1u
...
uint8_t MSS_SYS_flash_freeze(uint8_t options)
{
    ...
    MSS_COMBLK_send_cmd(flash_freeze_req,               /* p_cmd */
                        sizeof(flash_freeze_req),       /* cmd_size */
                        0,                              /* p_data */
                        0,                              /* data_size */
                        response,                       /* p_response */
    ...
    if((FLASHFREEZE_SHUTDOWN_CMD_LENGTH == actual_response_length) &&
       (FLASHFREEZE_SHUTDOWN_CMD == response[0]))
    {
...
uint8_t MSS_SYS_nrbg_reset(void)
{
    ...
    uint16_t actual_response_length;
    uint8_t reset_cmd[4];
    uint8_t response[DRBG_RESET_SERV_RESP_LENGTH];
    
    signal_request_start();
    
    reset_cmd[0] = DRBG_RESET_REQUEST_CMD;
    reset_cmd[1] = 0u;
    reset_cmd[2] = 0u;
    reset_cmd[3] = 0u;
    ...
    if((DRBG_RESET_SERV_RESP_LENGTH == actual_response_length) &&
       (DRBG_RESET_REQUEST_CMD == response[0]))
    {
        status = response[1];
    }
    else
 ...
 * See mss_sys_services.h for details.
 */
static uint8_t g_isp_response[ISP_PROGRAMMING_SERV_RESP_LENGTH];
...
)
{
    ...
    MSS_COMBLK_send_cmd(isp_prog_request,                   /* p_cmd */
                        sizeof(isp_prog_request),           /* cmd_size */
                        p_bitstream,                        /* p_data */
