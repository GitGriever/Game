/*------------------------------------------------------------------------------
 * Name:    UART_LPC17xx.c
 * Purpose: UART I/O functions for the Keil MCB1700 evaluation board 
 *          with the NXP LPC17xx microcontroller
 * Note(s):
 *------------------------------------------------------------------------------
...
int32_t UART_Uninitialize (void) {
  ...
  LPC_SC->PCONP &= ~(1 << 4);           /* Disable power to UART1             */
...
int32_t UART_SetConfiguration (UART_Configuration *config) {
  ...
  LPC_UARTx->FDR =  0;                       /* Fractional div not used (==1) */
