...
void KBD_Init (void) {
  ...
  PTC->PDDR &= ~(1 << 1);               /* PTC1 input                         */
  PTC->PDDR &= ~(1 << 2);               /* PTC2 input                         */

  PORTC->PCR[1]   = PORT_PCR_MUX(1)  |  /* PTC1 is GPIO pin                   */
                    PORT_PCR_PS_MASK |  /* pull up on PTC1                    */
                    PORT_PCR_PE_MASK;

  PORTC->PCR[2]   = PORT_PCR_MUX(1)  |  /* PTC2 is GPIO pin                   */
...
void KBD_UnInit (void) {
  PORTC->PCR[1] = 0;                    /* PTC1 is at reset state             */
  PORTC->PCR[2] = 0;                    /* PTC2 is at reset state             */
}
