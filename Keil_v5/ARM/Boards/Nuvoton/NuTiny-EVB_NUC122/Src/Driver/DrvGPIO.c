...
static void (*_pfGPCDCallback)(uint32_t u32GPCStatus, uint32_t u32GPDStatus);
...
{
    volatile uint32_t u32GPCStatus, u32GPDStatus;
    ...
    /* Call the callback function of GPIOAB interrupt */
    if ( _pfGPCDCallback )
        _pfGPCDCallback(u32GPCStatus, u32GPDStatus);    
}
...
int32_t DrvGPIO_EnableInt(E_DRVGPIO_PORT port, int32_t i32Bit, E_DRVGPIO_INT_TYPE TriggerType, E_DRVGPIO_INT_MODE Mode)
{
    ...
    if ((port == E_GPC) || (port == E_GPD))
    {
        NVIC_SetPriority(GPCD_IRQn, (1<<__NVIC_PRIO_BITS) - 2);
...
/*              pfGPCDCallback - [in], the function pointer of GPC/GPD/GPE callback function.              */
/* Returns:                                                                                                */
/*              None                                                                                       */
/* Description:                                                                                            */
...
void DrvGPIO_SetIntCallback(GPIO_GPAB_CALLBACK pfGPABCallback, GPIO_GPCD_CALLBACK pfGPCDCallback)
{
    ...
    _pfGPCDCallback  = (void (*)(uint32_t, uint32_t))pfGPCDCallback;
}
...
/*                                  GPB-4 as SPI1 SS1 for QFN33 package.                                   */
...
/*                                  GPB-8~10 as external counter input or toggle out pins.                 */
/*                              E_FUNC_PWM01 / E_FUNC_PWM23/ E_FUNC_PWM0 / E_FUNC_PWM1                     */
/*                                           / E_FUNC_PWM2 / E_FUNC_PWM3,                                  */
/*                                  GPA-12~15 as PWM0~3.                                                   */
/* Returns:                                                                                                */
/*              E_SUCCESS										Operation successful                       */
/*              E_DRVGPIO_ARGUMENT								Incorrect argument                         */
...
{  
    ...
    switch ( function )
	{
        ...
        case E_FUNC_SPI1:
        {
            ...
            outpw(&SYS->ALTMFP, inpw(&SYS->ALTMFP) & ~(0xF<<16));
        }break;

        case E_FUNC_SPI1_SS1_PB9:
        {
            outpw(&SYS->GPBMFP, inpw(&SYS->GPBMFP) | (0x1<<9));
            outpw(&SYS->ALTMFP, inpw(&SYS->ALTMFP) | (0x1<<1));
        }break;

        case E_FUNC_SPI1_SS1_PB4_QFN33:
        {
            outpw(&SYS->GPBMFP, inpw(&SYS->GPBMFP) | (0x1<<4));
            outpw(&SYS->ALTMFP, inpw(&SYS->ALTMFP) | (0x1<<15));
        }break;
