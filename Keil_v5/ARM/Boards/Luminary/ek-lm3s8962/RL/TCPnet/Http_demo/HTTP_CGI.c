  ...
  do {
    ...
    if (var[0] != 0) {
      ...
      if (str_scomp (var, "led0=on") == __TRUE) {
        ...
        str_copy (lcd_text[2], var+5);
        LCDupdate = __TRUE;
      }
      else if (str_scomp (var, "lcd2=") == __TRUE) {
        /* LCD Module Line 2 text. */
        str_copy (lcd_text[3], var+5);
        LCDupdate = __TRUE;
      }
    }
  }while (dat);
...
U16 cgi_func (U8 *env, U8 *buf, U16 buflen, U32 *pcgi) {
  ...
  switch (env[0]) {
    ...
    case 'f':
      ...
      switch (env[2]) {
        case '1':
          len = sprintf((char *)buf,(const char *)&env[4],lcd_text[2]);
          break;
        case '2':
          len = sprintf((char *)buf,(const char *)&env[4],lcd_text[3]);
          break;
      }
      break;
