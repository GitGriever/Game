...
__task void task_init (void)  {
  ...
  os_tsk_create (task_rtr_CAN,  4);     /* Start                RTR task      */
...
/*------------------------------------------------------------------------------
 *  Task: Sets answer to reception of remote transmission request periodically
 *----------------------------------------------------------------------------*/
...
__task void task_set_CAN (void)  {
  ...
  for (;;)  {
    ...
    CAN_set (2, &msg_set, 0x0F00);      /* Set data to answer on Remote req   */
...
/*------------------------------------------------------------------------------
 *  Task: Sends remote transmission request and receives response
 *----------------------------------------------------------------------------*/

__task void task_rtr_CAN (void)  {
  /* Initialize message  = { ID, {data[0]             ..                 data[7]}, LEN, CHANNEL,          FORMAT,         TYPE } */
  CAN_msg msg_rtr        = { 33, {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},   1,       3, STANDARD_FORMAT, REMOTE_FRAME };
  CAN_msg msg_rece;

  for (;;)  {
    CAN_request (2, &msg_rtr, 0x00FF);  /* Send request on controller 2, ch 3 */
