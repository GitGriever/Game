...
#define MSC_LUN_MAX                   1    /* number of used LUNs */
#define MSC_LUN_LAST   (MSC_LUN_MAX - 1)   /* LUN start with '0' */
...
BOOL MSC_GetMaxLUN (void) {
  ...
  EP0Buf[0] = MSC_LUN_LAST;                /* one LUN associated with this device */
  return (__TRUE);
}
...
void MSC_BulkOut (void) {
  ...
  switch (BulkStage) {
    ...
    case MSC_BS_DATA_OUT:
      ...
      break;
    case MSC_BS_CSW:
//      MSC_GetCBW();  /* ! CBW is received but stage is still BS_CSW. I am not shure why this happens */
      break;
    default:
      MSC_SetStallEP(MSC_EP_OUT);
      CSW.bStatus = CSW_PHASE_ERROR;
