...
#define SELECT          GPIO_PIN_4 /* PG4 */
#define UP              GPIO_PIN_4 /* PF4 */
#define DOWN            GPIO_PIN_5 /* PF5 */
#define LEFT            GPIO_PIN_6 /* PF6 */
#define RIGHT           GPIO_PIN_7 /* PF7 */
...
void SysTick_Handler (void) {
  ...
  if (GPIOPinRead(GPIO_PORTG_BASE, SELECT)) {
    Time++;
  }
  ...
  Buttons = GPIOPinRead(GPIO_PORTF_BASE, BUTTON);
  TraceB  = Buttons;
}
...
int main (void) {
  ...
  GPIODirModeSet  (GPIO_PORTF_BASE, BUTTON, GPIO_DIR_MODE_IN);
  GPIOPadConfigSet(GPIO_PORTG_BASE, SELECT, GPIO_STRENGTH_2MA, GPIO_PIN_TYPE_STD_WPU);
  GPIODirModeSet  (GPIO_PORTG_BASE, SELECT, GPIO_DIR_MODE_IN);
