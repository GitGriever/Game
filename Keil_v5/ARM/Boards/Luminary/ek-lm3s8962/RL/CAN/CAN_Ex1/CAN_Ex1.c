...
#define LED      GPIO_PIN_0           /* STATUS LED pin                      */
...
void Out_Init (void) {
  ...
  sprintf((char *)lcd_text[1],"   EK-LM3S8962 RL    ");
  sprintf((char *)lcd_text[2],"   CAN_Ex1 example   ");
  sprintf((char *)lcd_text[3],"                     ");
  sprintf((char *)lcd_text[4],"  Transmitted : 0x00 ");
  sprintf((char *)lcd_text[5],"  Received    : 0x00 ");
  sprintf((char *)lcd_text[6],"  TXed msgs   :    0 ");
...
void Out_Val (void) {
  ...
  GPIOPinWrite (GPIO_PORTF_BASE, LED, (led_blink < 1) ? LED : 0);

  /* Update transmitted and received values on LCD                           */
  sprintf((char *)lcd_text[4],"%.2X", Tx_val);
  sprintf((char *)lcd_text[5],"%.2X", Rx_val);
  sprintf((char *)lcd_text[6],"% 4i", Tx_cnt);
  sprintf((char *)lcd_text[7],"% 4i", Rx_cnt);
  RIT128x96x4StringDraw ((const char *)&lcd_text[4], 18*6, 10*4+6, 11);
