 ...
 * Note(s): used on STM320518-EVAL board
 *----------------------------------------------------------------------------
...
#define PIN_CS      (1UL << 4)
#define PIN_CLK     (1UL << 5)
#define PIN_DAT     (1UL << 7)
...
#define LCD_CS(x)   ((x)        ? (GPIOF->BSRR   = (PIN_CS   <<    0)) : (GPIOF->BSRR   = (PIN_CS    <<   16)) );
#define LCD_CLK(x)  ((x)        ? (GPIOA->BSRR   = (PIN_CLK  <<    0)) : (GPIOA->BSRR   = (PIN_CLK   <<   16)) );
#define LCD_DAT(x)  ((x)        ? (GPIOA->BSRR   = (PIN_DAT  <<    0)) : (GPIOA->BSRR   = (PIN_DAT   <<   16)) );

#define DAT_MODE(x)  (x == OUT) ? (GPIOB->BSRR   = ((1ul<<2) <<    0)) : (GPIOB->BSRR   =  ((1ul<<2) <<   16)); \
                     (x == OUT) ? (GPIOA->MODER |= ( 1UL     << 2* 7)) : (GPIOA->MODER &= ~( 3UL     << 2* 7));
#define BUS_VAL()   ((GPIOA->IDR & PIN_DAT) != 0)
...
static unsigned short rd_id_man (void) {
  ...
  GPIOA->MODER   |=  (( 1UL << (2* 5)) |
                      ( 1UL << (2* 6)) |
                      ( 1UL << (2* 7))  );       /* Gen. purpose output mode  */

  /* The direction of this voltage translator is controlled by IO PB2
    (IO PA7 acts as MOSI when PB2 is high or as MISO when PB2 is LOW)         */
  GPIOB->MODER   &= ~(( 3UL << (2* 2))   );
...
/* SPI1_SCK, SPI1_MISO, SPI1_MOSI are SPI pins                                */
  ...
  GPIOA->MODER   |=  (( 2UL << (2* 5)) |
                      ( 2UL << (2* 6)) |
                      ( 2UL << (2* 7))  );       /* Alternate Function mode   */
...
void GLCD_Init (void) {
  ...
  /* Enable clock for GPIOA, B, F and SPI1                                    */
  RCC->AHBENR  |= (( 1UL << 22) | ( 1UL << 21) | ( 1UL << 17));
  RCC->APB2ENR |=  ( 1UL << 12);

  /* NCS is PF4, GPIO output set to high                                      */
  GPIOF->ODR     |=  ( 1UL << (   4));           /* set Pin to High           */
  GPIOF->MODER   &= ~( 3UL << (2* 4));
  GPIOF->MODER   |=  ( 1UL << (2* 4));           /* Gen. purpose output mode  */
  ...
  GPIOA->AFR[0]  &= ~((15UL << (4* 5)) |
                      (15UL << (4* 6)) |
                      (15UL << (4* 7))  );       /* Alternate Function 0      */
