...
#if   (defined ( __CC_ARM ))
  #define __RAMFUNC                   /* __TEST   KEIL */
#elif (defined (__ICCARM__))
  #define __RAMFUNC  __ramfunc
#else
  /* only EWARM, MDK-ARM supported */
#endif
...
/**
  ...
  * @retval None
  */
__RAMFUNC void RAM_PWR_LowPowerRunModeCmd(FunctionalState NewState)
{
...
/**
  ...
  * @retval None
  */
__RAMFUNC void EnterLPRUNModeRAM(void)
{
  ...
  do{
    ...
    __NOP();  __NOP();    __NOP();  __NOP();
    __NOP();  __NOP();    __NOP();  __NOP();
  } while((BUTTON_GPIO_PORT->IDR & USER_GPIO_PIN) == 0);
...
/**
  ...
  * @retval None
  */
__RAMFUNC void EnterLPSLEEPModeRAM(void)
{
  uint32_t tmpreg = 0;
...
__RAMFUNC void EnterSLEEPModeRAM(void)
{
  uint32_t tmpreg = 0;
