...
void JOY_init(void) {
  ...
  GPIOG->CRH   &= ~0xFFF0F000;
  GPIOG->CRH   |=  0x44404000;
}
...
int main (void) {
  ...
  while (1) {                           /* Loop forever                       */
    ...
    if (GPIOG->IDR & (1 << 11)) joy |= (1 << 3);  /* Joystick down            */
    if (GPIOG->IDR & (1 <<  7)) joy |= (1 << 4);  /* Joystick select          */
    joy ^= 0x1F;

    /* Button inputs                                                          */
    but = 0;
    if (GPIOG->IDR & (1 <<  8)) but |= (1 << 0);  /* Button User (Key)        */
    but ^= 0x01;
    ...
    if (but ^ but_) {                    /* Show buttons status                */
      ...
      if (but & (1 << 0)) {
        GLCD_SetTextColor(Red);
        GLCD_DisplayString(6,  9, __FI, "KEY");
      } else {
