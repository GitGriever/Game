 ...
 *      RL-ARM - FlashFS
 *-----------------------------------------------------------------------------
 *      Name:    SDIO_STM32F2xx.c
 *      Purpose: SD/SDIO MMC Interface Driver for ST STM32F2xx
 *      Rev.:    V4.74
 *-----------------------------------------------------------------------------
...
static BOOL BusMode (U32 mode) {
  ...
  switch (mode) {
    case BUS_OPEN_DRAIN:
      GPIOD->OTYPER  |=  0x00000004;    /* Configure as open-drain            */
      return (__TRUE);

    case BUS_PUSH_PULL:
      GPIOD->OTYPER  &= ~0x00000004;    /* Configure as push-pull pin         */
      return (__TRUE);
...
static BOOL ReadBlock (U32 bl, U8 *buf, U32 cnt) {
  ...
  /* Enable data transfer and start DMA */
  SDIO->DCTRL = 0x9B;
...
static BOOL SetDma (U32 mode, U8 *buf, U32 cnt) {
  ...
  DMA2_Stream3->CR &= ~0x01;                   /* Disable stream 3            */

  DMA2_Stream3->PAR  = (U32)&(SDIO->FIFO);
  DMA2_Stream3->M0AR = (U32)buf;  
  DMA2_Stream3->NDTR = 0;
  DMA2_Stream3->FCR |= (1 << 2) |              /* Disable direct mode         */
  ...
  DMA2_Stream3->CR |= 0x01;                    /* Enable stream 3             */
