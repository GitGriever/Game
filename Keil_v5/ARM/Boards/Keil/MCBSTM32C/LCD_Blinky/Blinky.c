    ...
    switch (state) {
      ...
      case 1:                           /* Display rotating ARM logo          */
        if (ClockANI && (pic < 40)) {
          ClockANI = 0;
          if (pic++ >= 39) state++;
          GLCD_Bitmap (99, 99, 120, 45, &ARM_Ani_16bpp[(pic&7)*(120*45*2)]);
        }
        break;
      ...
      case 2:                           /* Display static text                */
        ...
        GLCD_DisplayString(0, 0, 1, "   MCBSTM32C Demo    ");
        GLCD_SetTextColor(Blue);
        GLCD_SetBackColor(White);
        GLCD_DisplayString(2, 0, 1, "AD value:            ");
        GLCD_DisplayString(3, 0, 1, "Buttons :            ");
        GLCD_DisplayString(5, 0, 1, "Joystick:            ");
        GLCD_Bitmap(  9, 176, 120, 48, Button_16bpp);
        GLCD_Bitmap(209, 178,  26, 40, Bulb_16bpp);
      ...
      case 3:                           /* Main demo                          */
        ...
        if (ad_val ^ ad_val_) {         /* Show AD value bargraph             */
          ...
          GLCD_Bargraph (144, 2*24, 160, 20, (ad_val>>2));
          ad_val_ = ad_val;
        }
        if (but ^ but_) {               /* Show buttons status                */
          GLCD_SetTextColor(Black);
          if (  but & (1 << 0) ) GLCD_DisplayString(3,  9, 1, "S1  ");
          if (  but & (1 << 1) ) GLCD_DisplayString(3, 13, 1, "S2  ");
          if (  but & (1 << 2) ) GLCD_DisplayString(3, 17, 1, "S3");
        ...
        if (joy ^ joy_) {               /* Show joystick status               */
          GLCD_Bitmap(196+ 0, 103+20, 19, 14, &Arrows_16bpp_red[(((joy>>0)&1)*2416)]);
          GLCD_Bitmap(196+35, 103+20, 19, 14, &Arrows_16bpp_red[(((joy>>1)&1)*2416)+1*532]);
          GLCD_Bitmap(196+20, 103+ 0, 14, 19, &Arrows_16bpp_red[(((joy>>2)&1)*2416)+2*532]);
          GLCD_Bitmap(196+20, 103+35, 14, 19, &Arrows_16bpp_red[(((joy>>3)&1)*2416)+3*532]);
          GLCD_Bitmap(196+21, 103+21, 12, 12, &Arrows_16bpp_red[(((joy>>4)&1)*2416)+4*532]);
          joy_ = joy;
        }
        if ((tch_on ^ tch_on_) &&       /* Show touch button status           */ 
          ...
          if ((tch_x >= 200) && (tch_x <= 1300) &&
            ...
            GLCD_Bitmap(  9, 176, 120, 48, &Button_16bpp[tch_on*11520]);
            if (tch_on) {
              light_on ^= 1;
              GLCD_Bitmap(209, 178, 26, 40, &Bulb_16bpp[light_on*2080]);
            }
          }
          if (!tch_on) {
            GLCD_Bitmap(  9, 176, 120, 48, &Button_16bpp[tch_on*11520]);
