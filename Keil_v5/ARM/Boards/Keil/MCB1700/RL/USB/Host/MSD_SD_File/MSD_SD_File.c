...
static void init_display (void) {
  ...
  GLCD_DisplayString (3, 0, 1, "    MSD_SD_File     ");
...
/*------------------------------------------------------------------------------
 *        Handle drive connections and initialization
 *----------------------------------------------------------------------------*/
static void  con_drv (void) {
  static U8  ini_m  = 0x80, ini_m_ex = 0xFF;
  static U8  ini_u  = 0x80, ini_u_ex = 0xFF;
         U8  con_dev   = 0;
         int key;

  ini_m = finit ("M:");                 /* Drive m: initialization and status */
  if (ini_m ^ ini_m_ex) {
    if (ini_m > 1) {                    /* If drive m: is unformatted         */
      printf ("\nSD/MMC Card is Unformatted");
      printf ("\nFormat the SD/MMC Card? [Y/N]\n");
      while ((key = getkey()) == -1);
      if (key == 'y' || key == 'Y') {
        /* Format the Memory Device with Label "KEIL". "*/
        if (fformat ("M: KEIL") == 0) {
          printf ("SD/MMC Card Formatted.\n");
          printf ("SD/MMC Card Label is KEIL");
          ini_m = 0;
        } else {
          printf ("Formatting failed.\n");
          ini_m = 2;
    ...
    } else if (ini_m == 1) {            /* If drive m: is not connected       */
      funinit ("M:");
      printf ("\nInsert the SD/MMC Card ...\n");
    } else {                            /* If drive m: is connected and fmted */
      con_dev |= 1;
    }
    ini_m_ex = ini_m;
    fflush (stdout);
  ...
  if (ini_u) {                          /* Initial initialization of drive u: */
    ini_u = finit ("U:");
  } else if (!ini_u || (ini_u == ini_u_ex)) {
    usbh_engine(0);
    ini_u     = !usbh_msc_status(0, 0);
  }
  if (ini_u ^ ini_u_ex) {
    if (ini_u > 1) {                    /* If drive u: is unformatted         */
      printf ("\nMass Storage Device is Unformatted");
      printf ("\nFormat the Mass Storage Device? [Y/N]\n");
      while ((key = getkey()) == 0);
      if (key == 'y' || key == 'Y') {
        /* Format the Mass Storage Device with Label "KEIL". "*/
        if (fformat ("U: KEIL") == 0) {
          printf ("Mass Storage Device Formatted.\n");
          printf ("Mass Storage Device Label is KEIL");
          ini_u = 0;
        } else {
          printf ("Formatting failed.\n");
          ini_u = 2;
    ...
    } else if (ini_u == 1) {            /* If drive u: is not connected       */
      funinit ("U:");
      printf ("\nInsert the Mass Storage Device ...\n");
    } else {                            /* If drive u: is connected and fmted */
      con_dev |= 2;
    }
    ini_u_ex = ini_u;
    fflush (stdout);
...
int main (void) {
  ...
  con_drv();                                  /* Init and handle drv con      */
