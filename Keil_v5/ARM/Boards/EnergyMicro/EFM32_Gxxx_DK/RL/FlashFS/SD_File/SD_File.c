...
static void cmd_cid (char *par) {
  ...
  drive  = get_entry (par, &next);
  fatvol = ioc_getcb (drive);
  if (fatvol->drv->DeviceCtrl (DC_GETCID, &cid)) {
...
int main (void) {
  ...
  DVK_writeRegister(BC_SPI_CFG, 1);     /* Enable SPI access to MicroSD card */
  DVK_enablePeripheral(DVK_SPI);
  DVK_enablePeripheral(DVK_RS232A);     /* Enable access to Serial Port A    */
