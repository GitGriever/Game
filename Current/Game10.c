...
// Initalise Timer2 to trigger every few seconds(assuming 880Mhz PLL)
  ...
  TIMER1_TAILR_R = seconds * 80000000 - 1; 
...
//					Nokia5110_PrintBMP(Player.x, Pipe.yTop + d, dot, 0);
//					Nokia5110_PrintBMP(Player.x+12, Player.y - 8, dot, 0);
...
void Timer2A_Handler(void){ 
 ...
 TimerCount++;
		buttonPress = 0;
	//			Timer2A_Start();
	//	Nokia5110_Clear(); // makes it worse
//		Nokia5110_ClearBuffer();
//		Nokia5110_PrintBMP(Player.x, Player.y, Player.image[2], 0);
//		Nokia5110_PrintBMP(X_MIN, Y_MAX, Ground, 0);
...
//   Delay100ms(1); // stopped the flickering
	if ((SW2 == 0) && (SW1 != 0)) 
	{
		Delay100ms(0.1);
		GPIO_PORTF_DATA_R &= ~0x02; // turn off red led
...
void cdelay(unsigned long count)
{
    unsigned long i=0;
    for(i=0; i<count; i++);
}
