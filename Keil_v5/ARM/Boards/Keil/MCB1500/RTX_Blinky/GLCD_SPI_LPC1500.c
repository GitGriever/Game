/******************************************************************************/
/* GLCD_SPI_LPC1500.c: LPC1500 low level Graphic LCD (240x320 pixels) driven  */
/*                     with SPI functions                                     */
/******************************************************************************/
...
#define LCD_CS(x)   ((x) ? (LPC_GPIO_PORT->SET[0] = PIN_CS)    : (LPC_GPIO_PORT->CLR[0] = PIN_CS))
#define LCD_CLK(x)  ((x) ? (LPC_GPIO_PORT->SET[1] = PIN_CLK)   : (LPC_GPIO_PORT->CLR[1] = PIN_CLK))
#define LCD_DAT(x)  ((x) ? (LPC_GPIO_PORT->SET[1] = PIN_DAT)   : (LPC_GPIO_PORT->CLR[1] = PIN_DAT))

#define DAT_MODE(x) ((x == OUT) ? (LPC_GPIO_PORT->DIR[1] |= PIN_DAT) : (LPC_GPIO_PORT->DIR[1] &= ~PIN_DAT))
#define BUS_VAL()                ((LPC_GPIO_PORT->PIN[1]  & PIN_DAT) != 0)
...
static __inline unsigned char spi_tran (unsigned char byte) {
  ...
  LPC_SPI1->TXDAT = byte;
  while (!(LPC_SPI1->STAT & RNE));        /* Wait for send to finish            */
...
static unsigned short rd_id_man (void) {
  ...
  LPC_SWM->PINASSIGN5 |=  ((0xFF <<  0) |        /* SCK is GPIO               */
                           (0xFF <<  8) |        /* MOSI is GPIO              */
                           (0xFF << 16)  );      /* MISO is GPIO              */
  LPC_GPIO_PORT->DIR[1] |= PIN_CLK;              /* SCK pin is GPIO output    */
  ...
  LPC_GPIO_PORT->DIR[1] &= ~PIN_CLK;
  LPC_SWM->PINASSIGN5 &= ~((0xFF <<  0) |        /* clear SPI1 pin assignment */
                           (0xFF <<  8) |
                           (0xFF << 16)  );
  LPC_SWM->PINASSIGN5 |=  ((  42 <<  0) |        /* P1.10 SCK                 */
                           (  41 <<  8) |        /* P1.9  MOSI                */
                           (  43 << 16)  );      /* P1.11 MISO                */
...
void GLCD_Init (void) {
  ...
  LPC_SYSCON->SYSAHBCLKCTRL0 |= ((1UL << 15) |   /* enable clock for GPIO1    */
                                 (1UL << 14) |   /* enable clock for GPIO0    */
                                 (1UL << 12)  ); /* enable clock for SWM      */

  LPC_SYSCON->SYSAHBCLKCTRL1 |=  (1UL << 10);    /* enable clock for SPI1     */

  /* Configure the LCD Control pins  (Backlight)                              */
  LPC_GPIO_PORT->DIR[0] |= (1UL << 10);
  LPC_GPIO_PORT->CLR[0]  = (1UL << 10);
  ...
  /* Max. 50 MBit used for Data Transfer @ 72MHz                              */
  LPC_SPI1->CFG    = ((1UL <<  0) |              /* SPI enable                */
                      (1UL <<  2) |              /* master mode               */
                      (1UL <<  4) |              /* CPHA  = 1                 */
                      (1UL <<  5)  );            /* CPOL  = 1                 */
  LPC_SPI1->TXCTL =   (7UL << 24);               /* 8bit dataframe            */
  LPC_SPI1->DIV   =    2;                        /* clk = PCLK/3 = 24MHz      */
