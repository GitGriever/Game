...
#define LCD_CS(x)   ((x) ? (GPIOH->BSRRL = PIN_CS) : (GPIOH->BSRRH = PIN_CS));
...
void GLCD_Init (void) {
  ...
  /* Enable clock for GPIOH,E and SPI1                                        */
  RCC->AHBENR  |= 0x00000030;
  RCC->APB2ENR |= 0x00001000;

  /* NCS is PH2, GPIO output set to high                                      */
  GPIOH->ODR   |=  0x00000004;          /* set Pin to High                    */
  GPIOH->MODER &= ~0x00000030;
  GPIOH->MODER |=  0x00000010;          /* General purpose output mode        */

  /* SPI1_SCK, SPI1_MISO, SPI1_MOSI are SPI pins                              */
  GPIOE->OSPEEDR |= 0xFC000000;

  GPIOE->MODER  &= ~0xFC000000;
  GPIOE->MODER  |=  0xA8000000;         /* set Alternate Function mode        */
  GPIOE->AFR[1] &= ~0xFFF00000;         /* Clear AFIO bits                    */
  GPIOE->AFR[1] |=  0x55500000;

  /* Enable SPI in Master Mode, CPOL=1, CPHA=1                                */
  /* Max. 18 MBit used for Data Transfer @ 32MHz                              */
//  SPI1->CR1     = 0x0357;
  SPI1->CR1     = 0x0347;
  SPI1->CR2     = 0x0000;
