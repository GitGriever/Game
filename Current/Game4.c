...
void Timer2A_Handler(void){ 
  ...
  TimerCount++;
	 // Draw(); // removes duplicates
    // Move(); // less flickering but slower
  Semaphore = 1; // trigger
	if(Player.life > 0)
	{c++;}
}
...
{ buttonPress = 0; }
	...
	while((SW1!= 0) && (SW2 != 0)){}
	// while((buttonPress != 0) || (buttonPress != 1)){}
}

//							while(Pause == 1) // works if press sw1 -> sw2 -> sw1
//				{
//					if(SW1 == 0) 
//					{
//						Pause = 0;
...
void Score_Init(void){
		...
		// score = ((37+c) / 37); // 35
	// score = c/100; // c/37 for older timer
