...
void ADC_Init (void) {
  ...
  RCC->AHBENR  |= ( 1UL <<    5);         /* Enable GPIOF clock               */
  GPIOF->MODER |= ( 3UL << 2*10);         /* PF.10 is Analog mode             */
  ...
  ADC1->SQR5    = (31UL <<  0);           /* 1. conversion = channel 18       */
  ADC1->SMPR0   = ( 4UL <<  3);           /* Chn 31 sample time is 48 cyc     */
  ADC1->SMPR1   =  0;                     /* Clear register                   */
  ADC1->SMPR2   =  0;                     /* Clear register                   */
  ADC1->SMPR3   =  0;                     /* Clear register                   */
