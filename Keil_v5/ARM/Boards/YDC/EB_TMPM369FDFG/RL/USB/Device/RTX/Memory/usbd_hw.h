...
/**
  * @brief USBD PLL and Clock source to Host or Device On/Off Enum
  */
    typedef enum {
        USBD_OFF = 0U,
        USBD_ON = 1U        
    } USBD_ONOFF;
#define IS_USBD_ONOFF(param)      ((param) <= USBD_ON)


/**
  * @brief USB PLL multiplier Enum For Different Clock Input
  */
    typedef enum {
        /* select this will also set to use external USBECLK for USB */
        USBD_INPUT_EXT = (uint32_t)(0x0000U),   
        
        /* select the below 3 items will also set to use PLL clock for USB */
        USBD_INPUT_8M  = (uint32_t)((0x5917U << 1U) + 1U),
        USBD_INPUT_12M = (uint32_t)((0x5A0FU << 1U) + 1U),
        USBD_INPUT_16M = (uint32_t)((0x720BU << 1U) + 1U)                       
    } USBD_PLLMULTIPLIER;
#define IS_USBD_PLLMULTIPLIER(param)  (((param) == USBD_INPUT_EXT)  || \
                                        ((param) == USBD_INPUT_8M)  || \
                                        ((param) == USBD_INPUT_12M) || \
                                        ((param) == USBD_INPUT_16M))    
