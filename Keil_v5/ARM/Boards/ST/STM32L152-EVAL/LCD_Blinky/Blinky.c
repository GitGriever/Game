...
int main (void) {
  ...
  GLCD_DisplayString(5, 0, __FI, "Buttons :            ");
...
//    but ^= 0x01;
    ...
    if (ad_val ^ ad_val_) {         /* Show AD value bargraph             */
      ...
      GLCD_Bargraph (144, 4*24, 160, 20, (ad_val>>2));
    ...
    if (but ^ but_) {               /* Show buttons status                */
      if (but & (1 << 0)) {
        ...
        GLCD_DisplayString(5,  9, __FI, "Key");
      } else {
    ...
    if (joy ^ joy_) {               /* Show joystick status               */
      if (joy & (1 << 0)) {         /* check  "Left" Key */
        GLCD_SetTextColor(Red);
        GLCD_DisplayChar(7,  9, __FI, 0x80+ 9);
      } else {
        GLCD_SetTextColor(LightGrey);
        GLCD_DisplayChar(7,  9, __FI, 0x80+ 8);
      }
      if (joy & (1 << 1)) {         /* check "Right" Key */
        GLCD_SetTextColor(Red);
        GLCD_DisplayChar(7, 11, __FI, 0x80+11);
      } else {
        GLCD_SetTextColor(LightGrey);
        GLCD_DisplayChar(7, 11, __FI, 0x80+10);
      }
      if (joy & (1 << 2)) {        /* check "Up" Key */
        GLCD_SetTextColor(Red);
        GLCD_DisplayChar(6, 10, __FI, 0x80+ 5);
      } else {
        GLCD_SetTextColor(LightGrey);
        GLCD_DisplayChar(6, 10, __FI, 0x80+ 4);
      }
      if (joy & (1 << 3)) {        /* check "Down" Key */
        GLCD_SetTextColor(Red);
        GLCD_DisplayChar(8, 10, __FI, 0x80+ 7);
      } else {
        GLCD_SetTextColor(LightGrey);
        GLCD_DisplayChar(8, 10, __FI, 0x80+ 6);
      }
      if (joy & (1 << 4)) {       /* check "Sel" Key */
        GLCD_SetTextColor(Red);
        GLCD_DisplayChar(7, 10, __FI, 0x80+ 1);
      } else {
        GLCD_SetTextColor(LightGrey);
        GLCD_DisplayChar(7, 10, __FI, 0x80+ 0);
      }
    ...
    if (ClockANI) {
      ClockANI = 0;
      if (pic++ > 8) pic = 0;
        GLCD_Bitmap (99, 34, 120, 45, &ARM_Ani_16bpp[pic*(120*45*2)]);
    }
