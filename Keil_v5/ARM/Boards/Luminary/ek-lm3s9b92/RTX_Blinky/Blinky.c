...
#define LED_PD0    (1UL<<0)
const unsigned long led_mask[] = { LED_PD0 };
 ...
 *----------------------------------------------------------------------------*/
void LED_init(void) {
  SYSCTL->RCGC2 |= (1 << 3);             /* Enable GPIOD clock               */
  __nop(); __nop(); __nop(); __nop();    /* Wait for peripheral ready        */
  GPIOD->DEN    |=  LED_PD0;             /* GPIOD.0 digital enable           */
  GPIOD->DIR    |=  LED_PD0;             /* GPIOD.0 output direction         */
}
