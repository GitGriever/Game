 ...
 *----------------------------------------------------------------------------*/
__task void file_man (void) {
  BOOL prompt = __TRUE;
  char *sp,*cp,*next;
  U32 i, active = 1;
  
  while (1) {
    /* Check if interface switching was requested */
    if (os_evt_wait_and (EVT_SW, 0) == OS_R_EVT) {
      active ^= 1;
      if (active) {
        ...
        printf (help);
        prompt = __TRUE;
        init_fs ();                           /* ReInit File System           */
      }
      else os_evt_set (EVT_SW, tid_sel);
    }
    ...
    if (active) {
      ...
      if (getline (in_line, sizeof (in_line))) {
        ...
        for (i = 0; i < CMD_COUNT; i++) {
          ...
          init_fs();                          /* Check if card is removed     */
