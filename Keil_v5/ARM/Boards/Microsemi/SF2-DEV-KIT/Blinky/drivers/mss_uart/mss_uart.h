 ...
 *
 * G4Main Microcontroller Subsystem UART bare metal software driver public API.
 *
 * SVN $Revision: 4578 $
 * SVN $Date: 2012-08-28 12:31:47 +0100 (Tue, 28 Aug 2012) $
 */
/*=========================================================================*//**
  @mainpage G4Main MSS UART Bare Metal Driver.

  @section intro_sec Introduction
  The G4Main MicroController Subsystem (MSS) includes two UART peripherals
  ...
  with the exception of the G4Main IOMUX configuration. G4Main allows
  ...
  The MSS UARTs serial signals are routed through IOMUXes to the G4Main
  ...
  on IOMUX, refer to the IOMUX section of the G4Main Datasheet.
  The base address, register addresses, and interrupt number assignment for the
  MSS UART blocks are defined as constants in the G4Main CMSIS-PAL You must
  ensure that the G4Main CMSIS-PAL is either included in the software tool
  ...
  MSS_UART_function_name( &g_mss_uart1, ... ). The MSS_UART_init() disables 
  the fractional baud rate mode and configures the integer baud value based 
  on the input baud rate parameter.  This function disables LIN, IrDA and 
  SmartCard modes.

  For LIN mode of operation, the driver is initialized through a call to 
  the MSS_UART_lin_init() function. This function takes the LIN node's 
  configuration as parameters. The MSS_UART_lin_init() function must be 
  called before any other LIN specific functions of the driver can be called. 

  For Irda mode of operation, the driver is initialized through a call to 
  the MSS_UART_irda_init() function. This function takes the IrDA node's 
  configuration as parameters. The MSS_UART_irda_init() function must be 
  called before any other IrDA specific functions of the driver can be called. 

  For Smartcard or ISO 7816 mode of operation, the driver is initialized 
  through a call to the MSS_UART_smartcard_init() function. This function 
  takes the smartcard configuration as parameters. The 
  MSS_UART_smartcard_init() function must be called before any other 
  smartcard specific functions of the driver can be called.
  ...
  Note:     The G4Main CMSIS-PAL defines the UART0_IRQHandler() and
  ...
  The G4MainTM MSS UART supports features like 9-bit mode operations, MSB or 
  LSB first for transmitter and receiver, setting address to the node, receiver 
  time-out and transmitter time guard etc. This dirver provides interface 
  functions to configure all the features supported by the MSS UART hardware. 
  Refer Table 1 for the list of common configuration functions.
 *//*=========================================================================*/
...
/***************************************************************************//**
  MSS UART Interrupts
  This enumeration specifies the interrupt masks to enable and disable MSS UART 
  interrupts. They are used to build the value of the irq_mask parameter for the
 ...
 */
typedef enum{
    MSS_UART_RBF_IRQ    = 0x001,
    MSS_UART_TBE_IRQ    = 0x002,
    MSS_UART_LS_IRQ     = 0x004,
    MSS_UART_MS_IRQ     = 0x008,
    MSS_UART_RTO_IRQ    = 0x010,
    MSS_UART_NACK_IRQ   = 0x020,
    MSS_UART_PIDPE_IRQ  = 0x040,
    MSS_UART_LINB_IRQ   = 0x080,
    MSS_UART_LINS_IRQ   = 0x100,
    MSS_UART_INVALID_IRQ = UINT16_MAX
}mss_uart_irq_t;
...
/***************************************************************************//**
  ...
  loopback parameter. Use MSS_UART_LOCAL_LOOPBACK_ON to set up the UART to locally
  loopback its Tx and Rx lines. Use MSS_UART_REMOTE_LOOPBACK_ON to set up the UART
...
typedef enum {
    ASYNC_MODE,
    SYNC_SLAVE_POS_EDGE_CLK,
    SYNC_SLAVE_NEG_EDGE_CLK,
    SYNC_MASTER_POS_EDGE_CLK,
    SYNC_MASTER_NEG_EDGE_CLK,
    INVALID_SYNC_MODE
...
/***************************************************************************//**
  The MSS_UART_init() function initializes and configures one of the G4Main
...
/***************************************************************************//**
  ...
  Example:
  @code
    MSS_UART_set_filter_length( &g_mss_uart0, LEN2 );
  @endcode
 */
void
...
/***************************************************************************//**
  ...
  Example:
  @code
    MSS_UART_set_ready_mode( &g_mss_uart0, READY_MODE0 );    
  @endcode
 */
void 
...
/***************************************************************************//**
  The MSS_UART_config_fractional_baud() function is used to configure fractional 
  baud rate to the MSS UART referenced by the parameter this_uart. This function 
  enables the fractional baud-rate mode for the UART instance, which was already 
  initialized by one of the the MSS_UART_init(), MSS_UART_lin_init(), 
  MSS_UART_irda_init() and MSS_UART_smartcard_init() function for integer 
  baudrate mode of operation. 
  ...
  @param baudrate
    The baudrate parameter is baud rate to be configured to the MSS UART 
    instance referenced by this_uart parameter. This can be any other value 
    apart from the defined baud rates handled by this driver.
  ...
  Example:
  @code
    MSS_UART_config_fractional_baud( &g_mss_uart0, 9000 );
  @endcode
 */
#if 0
/* 
    This function does not make sense to be part of the driver API. The driver
    will make use of fractional baud divisors when possible to achieve most
    accurate baud rate.
*/
void
MSS_UART_config_fractional_baud
(
    mss_uart_instance_t * this_uart,
    uint32_t baudrate    
);
...
/***************************************************************************//**
  The MSS_UART_config_integer_baud() function is used to configure interger 
  baudrate to the MSS UART referenced by the parameter this_uart. This function 
  disables the fractional baud-rate mode for the UART instance.  
  ...
  Example:
  @code
    MSS_UART_config_integer_baud( &g_mss_uart0, 9600 );
  @endcode
 */
#if 0
/*
    There is no need for this function in the driver's API. The baud rate
    configuration is performed as part of MSS_UART_init().
*/
void
MSS_UART_config_integer_baud
(
    mss_uart_instance_t * this_uart,
    uint32_t baudrate    
);
...
/***************************************************************************//**
  ...
  Example:
  @code
    MSS_UART_set_usart_mode( &g_mss_uart0, SYNC_SLAVE_POS_EDGE_CLK );
  @endcode
 */
void 
