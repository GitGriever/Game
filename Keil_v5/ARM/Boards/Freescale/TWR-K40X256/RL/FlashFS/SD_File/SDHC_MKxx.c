 ...
 *-----------------------------------------------------------------------------
 *      Name:    SDHC_MKxx.c
 *      Purpose: SD/SDIO MMC Interface Driver for Freescale MK60, MK50, ... MK10
 *      Rev.:    V4.23
 *-----------------------------------------------------------------------------
...
#include <MK40N512MD100.H>            /* MK40N512MD100 Definitions            */
#include "SDHC_MKxx.h"
...
static BOOL Init (void) {
  ...
  SIM->SOPT2  = (SIM->SOPT2 & ~SIM_SOPT2_SDHCSRC_MASK) | SIM_SOPT2_SDHCSRC(2);
  ...
  PORTA->PCR[16] = (PORTE->PCR[16]& ~PORT_PCR_MUX_MASK) | PORT_PCR_MUX(1) |
                    PORT_PCR_PE_MASK | PORT_PCR_PS_MASK;
  PTE->PDDR &= ~(1 << 27);
  PTA->PDDR &= ~(1 << 16);
