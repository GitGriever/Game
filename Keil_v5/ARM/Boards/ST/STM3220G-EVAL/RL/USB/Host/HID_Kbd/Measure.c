...
const char menu[] =
   ...
   "| Recorder.It is based on the STM32F207|\n"
   "| and records the state of the voltage |\n"
   "| on the analog input ADC1_12 .        |\n"
   "+ command ----+ function --------------+\n"
   "| R [n]       | read <n> records       |\n"
...
/******************************************************************************/
void ADC_Init (void) {
  /* ADC in pin is: PF9 as ADC3_IN7                                           */
  RCC->AHB1ENR |= ( 1 <<  5);                  /* enable clock for GPIOF      */
  GPIOF->MODER |= ( 3 << 18);                  /* Configure PC2 input.        */

  RCC->APB2ENR |= ( 1 << 10);                  /* enable clock for ADC3       */
  ADC3->SQR1    =  0;                          /* one conversion              */
  ADC3->SQR3    = ( 7 <<  0);                  /* set order to chn7           */
  ADC3->SMPR1   = ( 3 <<  6);                  /* sample time (56 cycles)     */ 

  ADC3->CR1     = ( 1 <<  8);                  /* independant mode, SCAN mode */
  ADC3->CR2     = ( 0 << 11)|                  /* data align right            */
                  ( 1 <<  1)|                  /* continuous conversion       */
                  ( 1 <<  0);                  /* enable ADC                  */ 

  ADC3->CR2    |= ( 1 << 30);                  /* start SW conversion         */
}
...
int ADC_GetConversion (void) {
  ...
  return (ADC3->DR & 0x0FFF);                  /* AD value (12 bit)           */
}
...
int main (void)  {                             /* main entry for program      */
  ...
  GLCD_Init ();                                /* Initialize the grapic LCD   */
