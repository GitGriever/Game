...
void ADC_Init (void) {
  ...
  PIOB->PIO_IDR  =
  PIOB->PIO_ODR  =
  PIOB->PIO_PER  = (PIO_PB1);             /* Setup Pins PB1 for ADC channel 5 */

  /* Setup and initialize ADC converter                                       */
  PMC->PMC_PCER0 =  ( 1 << ID_ADC);       /* enable clock for12-bit ADC       */
  ...
  ADC->ADC_CHER =  ( 1 <<  5);            /* enable channel 5                 */
...
#else
  ADC->ADC_IER  = ( 1 <<  5);             /* enable channel 5  EOC irq        */
#endif
...
void ADC_IRQHandler(void) {
  ...
  if (adc_isr & (1 << 5)) {               /* ADC chn3 EOC interrupt?          */
    AD_last = ADC->ADC_CDR[5];
    AD_done = 1;
  }
