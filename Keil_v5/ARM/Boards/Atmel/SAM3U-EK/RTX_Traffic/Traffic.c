...
void SetLights (U32 light, U32 on) {
   ...
   if (light == RED) {
      ...
      if (on == 0) {
         GLCD_DisplayChar(4, 9, __FI, 0x80+0);
      }
      else {
         GLCD_DisplayChar(4, 9, __FI, 0x80+1);
      }
   }
   if (light == YELLOW) {
      GLCD_SetTextColor(Yellow);
      if (on == 0) {
         GLCD_DisplayChar(5, 9, __FI, 0x80+0);
      }
      else {
         GLCD_DisplayChar(5, 9, __FI, 0x80+1);
      }
   }
   if (light == GREEN) {
      GLCD_SetTextColor(Green);
      if (on == 0) {
         GLCD_DisplayChar(6, 9, __FI, 0x80+0);
      }
      else {
         GLCD_DisplayChar(6, 9, __FI, 0x80+1);
      }
   }  
   if (light == STOP) {
      GLCD_SetTextColor(Red);
      if (on == 0) {
         GLCD_DisplayChar(5, 7, __FI, 0x80+12);
      }
      else {
         GLCD_DisplayChar(5, 7, __FI, 0x80+13);
      }
   }
   if (light == WALK) {
      GLCD_SetTextColor(Green);
      if (on == 0) {
         GLCD_DisplayChar(6, 7, __FI, 0x80+14);
      }
      else {
         GLCD_DisplayChar(6, 7, __FI, 0x80+15);
      }
   }
...
__task void lcd (void) {
  ...
  for (;;) {
    ...
    GLCD_DisplayString(0, 0, __FI, "    SAM3U-EK Demo   ");
