/*------------------------------------------------------------------------------
 * Name:    ADC.c
 * Purpose: Low level ADC functions for Freescale K20D7
 * Note(s): 
 *------------------------------------------------------------------------------
 ...
 *----------------------------------------------------------------------------*/
void ADC_Init (void) {
  /* Setup potentiometer pin ADC1_DM0 and A/D converter ADC1 */
  ...
  ADC1->SC1[0] = ( 1 << 6) |            /* Conversion complete interrupt enabled */
                 (19 << 0) ;            /* Select ADC_DM0 as a input          */
...
#endif
  ADC1->SC1[0] |= 19;                   /* Trigger software conversion        */  
}
