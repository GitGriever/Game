...
__task void task_init (void)  {
  ...
  os_tsk_create (task_get_CAN,  4);     /* Start                get task      */
  os_tsk_create (task_set_CAN , 3);     /* Start                set task      */
...
/*------------------------------------------------------------------------------
 *  Task: Set - Sets answer to reception of remote transmission request
 *----------------------------------------------------------------------------*/

__task void task_set_CAN (void)  {
  /* Initialize message  = { ID, {data[0]             ..                 data[7]}, LEN, CHANNEL,          FORMAT,       TYPE } */
  CAN_msg msg_set        = { 32, {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},   1,       1, STANDARD_FORMAT, DATA_FRAME };

  for (;;)  {
    msg_set.data[0] = In_Get ();        /* Data[0] field                      */

    CAN_set (3, &msg_set, 0x0F00);      /* Set data to answer on Remote req   */
    Tx_val = msg_set.data[0];
    os_dly_wait (100);                  /* Wait 1 second                      */
  }
}

/*------------------------------------------------------------------------------
 *  Task: Get - Sends remote transmission request and receives response
 *----------------------------------------------------------------------------*/

__task void task_get_CAN (void)  {
  /* Initialize message  = { ID, {data[0]             ..                 data[7]}, LEN, CHANNEL,          FORMAT,         TYPE } */
  CAN_msg msg_rtr        = { 33, {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},   1,       2, STANDARD_FORMAT, REMOTE_FRAME };
  CAN_msg msg_rece;

  for (;;)  {
    CAN_request (3, &msg_rtr, 0x00FF);  /* Send request on controller 3, ch 2 */
