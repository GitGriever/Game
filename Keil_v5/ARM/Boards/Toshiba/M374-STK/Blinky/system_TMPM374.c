...
/* Determine core clock frequency according to settings */
  ...
  #define __CORE_CLK   (0U)            /* Error, no XT, define nothing        */
          ...
          #define __CORE_CLK   (EXTALH * 8U / 2U)
        #elif ((SYSCR_Val & 7U) == 5U)          /* Gear -> fc/4     */
          #define __CORE_CLK   (EXTALH * 8U / 4U)
        #elif ((SYSCR_Val & 7U) == 6U)          /* Gear -> fc/8     */
          #define __CORE_CLK   (EXTALH * 8U / 8U)
        #elif ((SYSCR_Val & 7U) == 7U)          /* Gear -> fc/16    */
          #define __CORE_CLK   (EXTALH * 8U / 16U)
        #else                                   /* Gear -> reserved */
...
void SystemCoreClockUpdate (void)      /* Get Core Clock Frequency            */
{
  ...
  if (TSB_CG_CKSEL_SYSCK) {                /* If system clock is low-speed clock  */
    ...
    if (TSB_CG_OSCCR_OSCSEL){              /* If use High-speed oscillator 1(External) */
      if (TSB_CG_PLLSEL_PLLSEL && TSB_CG_OSCCR_PLLON) { /* If PLL used      */
        ...
        case 3U:
          ...
          break;
        case 4U:                                /* Gear -> fc/2     */
          SystemCoreClock = EXTALH * 8U / 2U;
          break;
        case 5U:                                /* Gear -> fc/4     */
          SystemCoreClock = EXTALH * 8U / 4U;
          break;
        case 6U:                                /* Gear -> fc/8     */
          SystemCoreClock = EXTALH * 8U / 8U;
          break;
        case 7U:                                /* Gear -> fc/16    */
          SystemCoreClock = EXTALH * 8U / 16U;
          break;
        default:
          SystemCoreClock = 0U;
          break;
