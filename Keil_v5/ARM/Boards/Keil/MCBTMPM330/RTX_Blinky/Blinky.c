...
#define LED_ENABLE  TSB_PI->CR   |=  0x07;     /* Enable LED pins as outputs */\
                    TSB_PI->DATA |=  0x07;     /* Turn all LEDs off          */
...
void LED_On(int led)
{
  if(led < 4) TSB_PI->DATA &= ~LED[led-1];    /* Turn LED on                */
}


void LED_Off(int led)
{
  if(led < 4) TSB_PI->DATA |=  LED[led-1];    /* Turn LED off               */
}
