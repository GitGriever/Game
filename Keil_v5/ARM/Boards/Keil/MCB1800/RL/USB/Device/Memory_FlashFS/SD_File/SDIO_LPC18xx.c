 ...
 *-----------------------------------------------------------------------------
 *      Name:    SDIO_LPC18xx.c
 *      Purpose: SD/SDIO MMC Interface Driver for NXP LPC18xx
 *      Rev.:    V4.74
 *-----------------------------------------------------------------------------
...
static SDIO_DMA_DESC DMADesc[4];
...
static BOOL BusSpeed (U32 kbaud) {
  ...
  LPC_SDMMC->CLKDIV = div >> 1;         /* Set clock divider                  */
  LPC_SDMMC->CLKENA = 1 | (1 << 16);    /* Stop clock when card in idle       */
