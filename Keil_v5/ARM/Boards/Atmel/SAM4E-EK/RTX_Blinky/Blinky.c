...
#include <RTL.h>
#include <SAM4E.H>                      /* ATSAM4E definitions               */
#include "GLCD.h"
#include "LED.h"
...
#define LED_A      0                    /* not enough LEDs available */
#define LED_B      1
#define LED_C      2
#define LED_D      3
#define LED_CLK    7
...
void LED_on  (unsigned char led) {
  ...
  GLCD_DisplayChar(4, led + 4, __FI, 0x80+1);       /* Circle Full        */
...
void LED_off (unsigned char led) {
  ...
  GLCD_SetBackColor(White);                         /* Set the Back Color */
  GLCD_SetTextColor(Green);                         /* Set the Text Color */
  GLCD_DisplayChar(4, led + 4, __FI, 0x80+0);       /* Circle Empty       */
  os_mut_release(&mut_GLCD);
}
...
__task void lcd (void) {
  ...
  for (;;) {
    ...
    GLCD_DisplayString(1, 0, __FI, (unsigned char *)"    RTX Blinky      ");
