/*----------------------------------------------------------------------------
 * Name:    GLCD_16bitIF_STM32L1xx.c
 * Purpose: low level Graphic LCD (320x240 pixels) driven with 16-bit parallel interface
 * Note(s): used on STM32L152-EVAL board
 *----------------------------------------------------------------------------
...
#define GLCD_BASE   (0x60000000UL | 0x0C000000UL)

#define LCD_REG16_R  (*((volatile unsigned short *)(GLCD_BASE  )))
#define LCD_DAT16_R  (*((volatile unsigned short *)(GLCD_BASE+2)))
#define LCD_REG16_W  (*((volatile unsigned short *)(GLCD_BASE+4)))
#define LCD_DAT16_W  (*((volatile unsigned short *)(GLCD_BASE+6)))
...
static __inline unsigned short rd_dat (void) {
  ...
  return (LCD_DAT16_R);                                    /* return value */
}
...
//  GPIOD->OSPEEDR  &= ~0xF03F0F0F;       /* Clear Bits                         */
  GPIOD->OSPEEDR  |=  0xF03F0F0F;       /* 40 MHz Fast speed                  */
...
//  GPIOE->OSPEEDR  &= ~0xFFFFC000;       /* Clear Bits                         */
  GPIOE->OSPEEDR  |=  0xFFFFC000;       /* 40 MHz Fast speed                  */
  ...
  /* PF.00(A0 (RS)), PF.01 (A1 (Level Shifter Direction)) */
  GPIOF->MODER    &= ~0x0000000F;       /* Clear Bits                         */
  GPIOF->MODER    |=  0x0000000A;       /* Alternate Function mode            */
//  GPIOF->OSPEEDR  &= ~0x0000000F;       /* Clear Bits                         */
  GPIOF->OSPEEDR  |=  0x0000000F;       /* 40 MHz Fast speed                  */
  GPIOF->AFR[0]   &= ~0x000000FF;       /* Clear Bits                         */
  GPIOF->AFR[0]   |=  0x000000CC;       /* Alternate Function mode AF12       */
